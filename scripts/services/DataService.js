...
const DataService = {
  _sendRequest(url, successCallback, method = 'GET') {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status != 200) {
        ...
        successCallback(responseData);// responseText -- текст ответа.
      }
    }
  },

  _sendMultpleRequests(urls, callback) {
  ...
  getCurrencies(callback) {
    ...
    DataService._sendRequest(COINS_URL, data => {
      const firstTenData = data.slice(0, 25);
  ...
  getCurrenciesPrices(data, callback) {
    ...
    DataService._sendMultpleRequests(Object.keys(coinsIdMap), coins => {
